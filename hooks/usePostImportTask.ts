  ...
  const startPolling = (taskId: string) => {
    ...
    const poll = async () => {
      try {
        ...
        // Get task status
        const task = await api.postImport.getImportTaskStatus.query(
          { taskId },
        ...
        console.log(`Polling attempt ${pollingAttemptsRef.current}, elapsed time: ${elapsedSeconds}s, next interval: ${interval}ms`)
