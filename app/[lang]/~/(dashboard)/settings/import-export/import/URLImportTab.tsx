...
import { Link, Globe, ArrowRight, AlertTriangle, Info, FileText, Search } from 'lucide-react'
...
export function URLImportTab({
  ...
  const shouldDisplayPostSelection = importTask?.status === 'completed' && importTask.result && importTask.result.length > 0
  const shouldDisplayEmptyResult = importTask?.status === 'completed' && (!importTask.result || importTask.result.length === 0)
  ...
  return (
    <div className="w-full flex flex-col space-y-5">
      ...
      {!isImporting && !shouldDisplayPostSelection && !shouldDisplayEmptyResult && (
